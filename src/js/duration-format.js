...
define([
     ...
     */
    function durationFormat(ms, strings, precision) {
        ...
        if(magnitude >= 86400e3) {
            round = ms / 86400e3;
            return Math.abs(round) === 1
                ? round + oneDay
                : round.toFixed(precision) + days;
        } else if(magnitude >= 3600e3) {
            round = ms / 3600e3;
            return Math.abs(round) === 1
                ? round + oneHour
                : round.toFixed(precision) + hours;
        } else if(magnitude >= 60e3) {
            round = ms / 60e3;
            return Math.abs(round) === 1
                ? round + oneMinute
                : round.toFixed(precision) + minutes;
